...
export function calculatePosition(input) {
  const { capital, entry, stop, riskPercent, leverage } = input;
  ...
  if (stop === entry) return { error: '开仓价与止损价不能相同。' };
  const direction = stop < entry ? 'long' : 'short';

  const stopAmount = capital * riskPercent / 100;
  ...
  return { direction, stopAmount, stopDistancePercent, margin, notional, quantity, capitalUsagePercent, accountRiskPercent, overCapital: margin > capital };
}
